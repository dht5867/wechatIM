  ...
  (function (UPNG, pako) {
    ...
    UPNG.toRGBA8.decodeImage = function (data, w, h, out) {
      ...
      if (ctype == 6) { // RGB + alpha
        ...
        if (depth == 16) for (var i = 0; i < area; i++) {
          ...
          bf[qi + 2] = data[ti + 4];
          bf[qi + 3] = 255;
          if (tr != -1 && rs(data, ti) == tr && rs(data, ti + 2) == tg && rs(data, ti + 4) == tb) bf[qi +
            3] = 0;
        }
    ...
    UPNG.encode.compress = function (bufs, w, h, ps, forGIF) {
      ...
      for (var j = 0; j < bufs.length; j++) // when not quantized, other frames can contain colors, that are not in an initial frame
      {
        ...
        for (var i = 0; i < img.length; i += 4) {
          var c = img32[i >> 2];
          if ((j == 0 || c != pr32[i >> 2]) && plte.length < 600 && cmap[c] == null) {
            cmap[c] = plte.length;
            plte.push(c);
          }
      ...
      for (var j = 0; j < bufs.length; j++) {
        ...
        if (cc <= 256) {
          ...
          for (var y = 0; y < nh; y++) {
            var i = y * bpl,
              ii = y * nw;
            if (depth == 1) for (var x = 0; x < nw; x++) nimg[i + (x >> 3)] |= (cmap[cimg32[ii + x]] << (7 -
              (x & 7)));
            if (depth == 2) for (var x = 0; x < nw; x++) nimg[i + (x >> 2)] |= (cmap[cimg32[ii + x]] << (6 -
              (x & 3) * 2));
            if (depth == 4) for (var x = 0; x < nw; x++) nimg[i + (x >> 1)] |= (cmap[cimg32[ii + x]] << (4 -
              (x & 1) * 4));
    ...
    UPNG.encode._filterLine = function (data, img, y, bpl, bpp, type) {
      ...
      if (type == 0) for (var x = 0; x < bpl; x++) data[di + x] = img[i + x];
        ...
        if (type == 3) for (var x = bpp; x < bpl; x++) data[di + x] = (img[i + x] - (img[i + x - bpp] >> 1) +
          256) & 255;
        if (type == 4) for (var x = bpp; x < bpl; x++) data[di + x] = (img[i + x] - paeth(img[i + x - bpp], 0,
          0) + 256) & 255;
      } else {
        if (type == 2) {
        ...
        if (type == 4) {
          for (var x = 0; x < bpp; x++) data[di + x] = (img[i + x] + 256 - paeth(0, img[i + x - bpl], 0)) &
            255;
          for (var x = bpp; x < bpl; x++) data[di + x] = (img[i + x] + 256 - paeth(img[i + x - bpp], img[i +
            x - bpl], img[i + x - bpp - bpl])) & 255;
        }
      }
    }
    ...
    UPNG.quantize.applyPalette = function (nimg, plte, pind) {
      ...
      for (var i = 0; i < area; i++) {
        ...
        nimg[qi + 0] = rnd(plte[qci + 0] * ia);
        nimg[qi + 1] = rnd(plte[qci + 1] * ia);
        nimg[qi + 2] = rnd(plte[qci + 2] * ia);
    ...
    UPNG.quantize.kMeans = function (nimg, plte, pind, ps, update) {
      ...
      var ndst = new Uint32Array(ps),
        nind = new Uint32Array(ps); // for each color, find the nearest color (in palette)
      for (var i = 0; i < ps; i++) {
        var qi = i << 2;
        var r = plte[qi],
          g = plte[qi + 1],
      ...
      var hist = new Uint32Array(ps),
        nplt = new Uint32Array(ps * 4); // classify each pixel
      for (var i = 0; i < area; i++) {
        var qi = i << 2;
        var r = nimg[qi],
          g = nimg[qi + 1],
      ...
      if (update) // update palette
        for (var i = 0; i < ps; i++) {
          var qi = i << 2,
            den = 1 / hist[i];
          plte[qi] = rnd(nplt[qi] * den);
          plte[qi + 1] = rnd(nplt[qi + 1] * den);
          plte[qi + 2] = rnd(nplt[qi + 2] * den);
          plte[qi + 3] = rnd(nplt[qi + 3] * den);
        }
    }
    ...
    UPNG.decode = function (buff) {
      ...
      while (true) {
        ...
        if (type == "IHDR") {
          ...
          else if (out.ctype == 2) out.tabs[type] = [rUs(data, offset), rUs(data, offset + 2), rUs(data,
            offset + 4)];
          ...
          else if (out.ctype == 2 || out.ctype == 6) out.tabs[type] = [rUs(data, offset), rUs(data, offset +
            2), rUs(data, offset + 4)];
    ...
    UPNG.decode._filterZero = function (data, out, off, w, h) {
      ...
      for (var y = 0; y < h; y++) {
        ...
        if (type == 0) for (var x = 0; x < bpl; x++) data[i + x] = data[di + x];
          ...
          if (type == 3) for (var x = bpp; x < bpl; x++) data[i + x] = (data[di + x] + (data[i + x - bpp] >>
            1)) & 255;
          if (type == 4) for (var x = bpp; x < bpl; x++) data[i + x] = (data[di + x] + paeth(data[i + x - bpp],
            0, 0)) & 255;
        } else {
          if (type == 2) {
          ...
          if (type == 3) {
            ...
            for (var x = bpp; x < bpl; x++) data[i + x] = (data[di + x] + ((data[i + x - bpl] + data[i + x -
              bpp]) >> 1)) & 255;
          }

          if (type == 4) {
            for (var x = 0; x < bpp; x++) data[i + x] = (data[di + x] + paeth(0, data[i + x - bpl], 0)) &
              255;
            for (var x = bpp; x < bpl; x++) data[i + x] = (data[di + x] + paeth(data[i + x - bpp], data[i +
              x - bpl], data[i + x - bpp - bpl])) & 255;
          }
        }
      }
      return data;
